...
params:add{ type = "number", id = "delay_shift_0", name = "delay_shift_0", min = -12, max = 12, default = 5 }
params:add{ type = "number", id = "delay_shift_1", name = "delay_shift_1", min = -12, max = 12, default = 2 }
params:add{ type = "number", id = "delay_shift_2", name = "delay_shift_2", min = -12, max = 12, default = -7 }

function init()
  crow.output[2].action = "pulse(0.1)"
  globals = {
    playback = false,
    generation = 0,
    numerator = 4,
...
crow.input[1].mode("stream", 0.01) -- or you can try crow.input[1].mode("stream", 0.001)
current = 1
...
vals = {0,0} -- two values in the history

function process_stream(v)
  vals[current] = v -- the current value should always be stored
  if math.abs(vals[current] - vals[util.wrap(current + 1,1,2)]) >= (1/12-0.01) then -- if the current value is over a semitone away from the last stored value...
    print(">>>> "..v) -- print the new value (replace this with whatever you wanna do with the new value)
    current = util.wrap(current + 1,1,2) -- increment the history index, with wrapping, so 1 -> 2 and 2 -> 1
    local crow_put = (math.floor(v * 12 + 0.5))
    local note = crow_put
    print(">>>>"..crow_put)
    local new_note = note_factory(crow_put, "wet", 127, globals.generation, music.default_note_duration)
...
function play_note(note)
  ...
  note.is_on = true
  local crow_note = (note.note / 12 - 3)
  print(crow_note)
  if crow_note < 10 then
    crow.output[1].volts = crow_note
...
function register_delay(note)
  local delay_pitch = note.note
  globals.delay_counter = (globals.delay_counter + 1) % 3
  ...
  --trying to add my pitch shifting shit
  if globals.delay_counter == 0 then
    delay_pitch = note.note + params:get("delay_shift_0")
  elseif globals.delay_counter == 1 then
    delay_pitch = note.note + params:get("delay_shift_1")
  elseif globals.delay_counter == 2 then
    delay_pitch = note.note + params:get("delay_shift_2")
  end
    if delay_velocity > 1 then -- 1 is an arbitrary threshold for silence
    local new_note = note_factory(delay_pitch, "wet", delay_velocity, delay_generation, music.default_note_duration)
    register_note(new_note)
  end
end
